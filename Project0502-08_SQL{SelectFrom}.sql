...
SELECT l.city, MIN(r.avgRating) AS 'The lowest average rating', 
	MAX(r.avgRating) AS 'The highest average rating', 
	AVG(r.avgRating) AS 'Average of average ratings'
FROM Restaurant r, Location l
Where r.restaurantID = l.restaurantID
GROUP BY l.city
ORDER BY l.city;

--How many restaurants does each category have in different cities? What is the average rating of each category in different cities?
SELECT ca.categoryName, COUNT(DISTINCT r.restaurantID) AS 'Amount of Restaurants', AVG(r.avgRating) AS 'Average Rating'
FROM Restaurant r, Location l, Classify cl, Category ca
WHERE r.restaurantID = l.restaurantID 
	AND r.restaurantID = cl.restaurantID
	AND cl.categoryID = ca.categoryID
	AND l.city = 'College Park'
GROUP BY ca.categoryName
ORDER BY COUNT(DISTINCT r.restaurantID) DESC;